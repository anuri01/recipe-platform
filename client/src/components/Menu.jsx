    ...
    <>
      ...
      <div className={`menu-container ${isLayerOpen ? 'open' : ''}`}>
        <div className="menu-header">
          <div className="util-group">
            {isLoggedIn ? (
              <>
                <h2>
                  <Link to="/profile" onClick={closeLayer} className="auth-link">
                    {user.username}
                  </Link>{' '}
                  님 안녕하세요!
                </h2>
              ...
              <>
                <h2>
                  <Link to="/login" onClick={closeLayer} className="auth-link">
                    로그인
                  </Link>{' '}
                  하세요.
                </h2>
                <p>
                  계정이 없다면?{' '}
                  <Link to="/signup" onClick={closeLayer} className="auth-link">
                    회원가입
                  </Link>
                </p>
              </>
          ...
          <button
            onClick={closeLayer}
            className={`close-btn ${!isLoggedIn ? 'position' : ''}`}>
            &times;
          </button>
        </div>
        <nav className="menu-nav">
          <ul>
            <li>
              <Link to="/" onClick={closeLayer}>
                메인으로
              </Link>
            </li>
            <li>
              <Link to="/recipes" onClick={closeLayer}>
                레시피 보기
              </Link>
            </li>
            <li>
              <Link to="/recipes/editor" onClick={closeLayer}>
                레시피 등록
              </Link>
            </li>
          </ul>
