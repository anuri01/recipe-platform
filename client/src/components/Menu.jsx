...
return (
    ...
    <div className={`menu-container ${isLayerOpen ? 'open' : ''}`}>
        <div className="menu-header">
            ...
            { isLoggedIn ? (
                <h2><Link to='/profile' className="auth-link">{'홍길동'/*{user.username}*/}</Link> 님 안녕하세요!</h2>
            ) : (
                <h2><Link to='/login' className="auth-link">로그인</Link> 하세요.</h2>
            )}
            { !isLoggedIn && <p>계정이 없다면? <Link to='/signup' className="auth-link">회원가입</Link></p>}
            </div>
