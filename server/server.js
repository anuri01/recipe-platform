...
app.post('/api/users/signup', async (req, res) => {
  try {
    const { username, password, name, email, address, phoneNumber, interests } =
      req.body
...
app.get('/api/recipes', async (req, res) => {
    try {
        ...
        let query = {}; // 빈 객체로 초기화
        if (keyword) {
            const searchRegex = new RegExp(keyword, 'i'); // 대소문자 구분 없는 정규식
            query = {
                $or: [
                    { title: { $regex: searchRegex } },
                    { 'content.description': { $regex: searchRegex } },
                    { 'content.ingredients': { $regex: searchRegex } },
                ],
            };  
        }
        const recipes = await Recipe.find(query).populate('creator', 'username').sort({createdAt: -1}); // 작성자 정보도 함께 가져오기
  ...
  async (req, res) => {
    try {
      ...
      const mainImageUrl = req.files.mainImage
        ? req.files.mainImage[0].location
        : null
      const recipeImageUrl = req.files.recipeImages
        ? req.files.recipeImages.map((file) => file.location)
        : []

      if (!title || !content || !category || !mainImageUrl) {
        return res
          .status(400)
          .json({ message: '필수 항목 입력을 확인해 주세요' })
      }
...
app.get('/api/recipes/:id', async (req, res) => {
  try {
    ...
    console.log('아이디', req.params.recipeId)
